import time
import smbus
from waterSys import Adafruit_I2C
...
class ADS1x15:
  ...
  def __init__(self, address=0x48, ic=__IC_ADS1015, debug=False):
      ...
      self.i2c = Adafruit_I2C.Adafruit_I2C(address)
      self.address = address
      self.debug = debug
      # Make sure the IC specified is valid
      if ((ic < self.__IC_ADS1015) | (ic > self.__IC_ADS1115)):
        if (self.debug):
          print ("ADS1x15: Invalid IC specfied: {0}".format(ic))
      else:
        self.ic = ic
  ...
  def readRaw(self, channel=0, pga=6144, sps=250):
      ...
      if (channel > 3):
        if (self.debug):
            print("ADS!x15: Invalid channel specified: {0}".format(channel))
        return -1
